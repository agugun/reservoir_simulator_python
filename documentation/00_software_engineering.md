...
2. **Just-In-Time (`@jit`) C++ Caching via XLA compiler**: 
   Because geological boundaries (`Grid`, `Rock`) and fluid dynamics (`Fluid` arrays) are constant properties over a time step, we rigorously isolate the pure mathematical execution limits natively. Upon the very first execution, JAX compiles the complete internal Newton solver iteration dynamically into an optimized C++ XLA binary matrix map, permanently bypassing slower Python loop interpretation.
3. **Structured Sparse Matrix Linear Inversions**:
   By mathematically packaging the JAX analytical derivatives as Compressed Sparse Row variables (`csr_matrix`), the native mathematical execution utilizing structural Direct Sparse Solver mappings (`scipy.sparse.linalg.spsolve`) essentially erases the $O(N^3)$ memory limitations of dense LAPACK inversions.
4. **Advanced PID-Target Time-Stepping**:
   To maximize simulation throughput, the simulator implements an industrial PID (Proportional-Integral-Derivative) controller for temporal discretization. Instead of blind multipliers, the system traces the exact non-linear convergence depth (Target: 4 iterations) and dynamically scales $\Delta t$ using an industrial scaling multiplier ($\alpha = \frac{T_{target} + \omega}{T_{actual} + \omega}$), gracefully navigating through difficult saturation "flashing" events without catastrophic divergence crashes.
5. **Native GPU Hardware Acceleration**:
   The entire numerical pipeline is **Hardware Agnostic**. By utilizing JAX as the primary computational substrate, the exact same Python source code transparently executes across **NVIDIA CUDA** backends. The XLA compiler unrolls the massive 3D residual matrices directly onto GPU arithmetic logic units (ALUs), enabling million-cell simulation capabilities on standard consumer hardware.
